...
public class BluetoothChatService extends Service {
    ...
    private BluetoothGattCallback callback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (newState == BluetoothGatt.STATE_CONNECTED) {
                ...
                android.util.Log.d(TAG, "Connection state changed.  New state: " + newState);
            }
        }
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                android.util.Log.d(TAG, "Service discovery completed!");
            }
            else {
                android.util.Log.d(TAG, "Service discovery failed with status: " + status);
            }
            ...
            if (!gatt.setCharacteristicNotification(rx, true)) {
                android.util.Log.d(TAG, "Couldn't set notifications for RX characteristic!");
            }
            ...
            if (rx.getDescriptor(CLIENT_UUID) != null) {
                ...
                if (!gatt.writeDescriptor(desc)) {
                    android.util.Log.d(TAG, "Couldn't write RX client descriptor value!");
                }
            }
            else {
                android.util.Log.d(TAG, "Couldn't get RX client descriptor!");
            }
        }
    ...
    private LeScanCallback scanCallback = new LeScanCallback() {
        ...
        public void onLeScan(BluetoothDevice bluetoothDevice, int i, byte[] bytes) {
            android.util.Log.d(TAG, "Found device: " + bluetoothDevice.getAddress());
            // Check if the device has the UART service.
            if (parseUUIDs(bytes).contains(UART_UUID)) {
                // Found a device, stop the scan.
                adapter.stopLeScan(scanCallback);
                android.util.Log.d(TAG, "Found UART service!");
    ...
    // OnDestroy, close the BTLE connection.
    @Override
